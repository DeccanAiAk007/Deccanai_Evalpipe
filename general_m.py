...
def build_generation_prompt(question):
    return f"""You are a reliable and concise academic assistant specialized in Physics, Chemistry, Mathematics, and Biology.

Answer the following question in exactly 3 to 4 academically accurate words. Use only verified subject knowledge. Avoid any elaboration, intuition, or vague phrasing.

If the question cannot be confidently answered with well-established principles from PCMB, respond with: "Not enough information".
...
def build_verification_prompt(question, human_answer, model_answer):
    return f"""
You are a highly competent and unbiased academic evaluator. Your task is to assess the quality of two responses—one written by a human, and one generated by a model—to a given question from Physics, Chemistry, Mathematics, or Biology.

You must evaluate both responses based on **five well-defined criteria**, using the definitions below. Your ratings must be fair, consistent, and grounded in rigorous academic knowledge and reasoning.
...
3. **Scientific Accuracy**: Is it factually correct, consistent with principles of PCMB, and academically sound?
4. **Hallucination**: Are any scientific claims unsupported, incorrect, or fabricated?
   → Full score (1.0) if no hallucination.
5. **Clarity**: Is the answer clear, precise, and unambiguous?

Scoring Guide (between 0 to 1):

- 0.0 — Poor:
Entirely incorrect, irrelevant, or misleading.
Includes hallucinated facts or unrelated information.
...
Shows some academic reasoning but lacks clarity and precision.

- Exactly 0.5 — Minimally Acceptable:
Somewhat helpful but basic or underdeveloped.
Lacks detail or specificity.
No serious errors, but not academically sufficient.
...
- 1.0 — Excellent:
Fully accurate, concise, and academically sound.
Clearly addresses the question with verified subject knowledge.
Free from hallucinations and ambiguity.
Trustworthy for scientific understanding or use.
...
- Focus on **semantic accuracy**, **academic soundness**, and **subject usefulness**.
- Penalize hallucinated facts or scientifically inaccurate statements.
...
if __name__ == "__main__":
    input_csv = "math_que.csv"
    output_csv = "math_ans.csv"
